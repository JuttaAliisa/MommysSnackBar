...
def consumption(usage_sheet, stock_sheet):
    ...
    print(Fore.BLUE)
    print('When taking a snack from the stock')
    print('please insert correct number for the snack')
    print(Style.RESET_ALL)
...
def recommendation(usage_sheet, stock_sheet, recommendation_sheet):
    ...
    print(Style.RESET_ALL)
    print("Please consider purchasing the following")
    print("snacks to make sure the stock lasts:")
    print(Fore.GREEN)
    ...
    recommendation_values = [usage - stock + 1 if stock == 0 else usage - stock
                             for usage, stock in zip(usage_values, stock_values)]
...
def start():
    ...
    while True:
        ...
        if user_choice == '1':
            ...
            restock(spreadsheet.worksheet('restock'),
                    spreadsheet.worksheet('usage'),
                    spreadsheet.worksheet('stock'))
        elif user_choice == '3':
            stock(spreadsheet.worksheet('stock'))
        elif user_choice == '4':
            recommendation(spreadsheet.worksheet('usage'),
                           spreadsheet.worksheet('stock'),
                           spreadsheet.worksheet('recommendation'))
        elif user_choice == '5':
...
def stocktaking(spreadsheet):
    """
    Stocktaking gives opportunity to reset the stock and usage in
    case the user feels like the numbers are not correct.
    This feature will delete usage, stock and
    ...
    while True:
        ...
        if user_choice == 'y':
            ...
            print("Please proceed to restocking the actual stock you have by pressing enter")
            input()
            restock(spreadsheet.worksheet('restock'),
                    spreadsheet.worksheet('usage'),
                    spreadsheet.worksheet('stock'))
